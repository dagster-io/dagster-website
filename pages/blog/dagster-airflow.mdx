...
<p align="center">
  <img src="/posts/dagster-airflow/data_lifecycle.png" alt="The Data Lifecycle" />
</p>
...
<p align="center">
  <img src="/posts/dagster-airflow/hello_world.png" alt="A simple pipeline." />
</p>
...
<p align="center">
  <img src="/posts/dagster-airflow/dagster_pipeline_screenshot.png" alt="A pipeline in Dagster." />
</p>
...
Dagster pipelines are graphs of Python functions focused on business logic. Dagster’s API enables a separation of concerns between compute and I/O, a necessary condition for testable data applications with fast developer feedback. Separation of compute and I/O at the graph level requires explicit support from the orchestration framework.

Dagster lets you provide pluggable resources to manage access to external systems, whose behavior can vary between different environments, such as development, staging, and production. These are used to model any heavyweight or side-effect-producing system dependency, not just I/O.
...
Dagster’s long running process, the daemon, is responsible for scheduling runs, not tasks. This allows for a layered approach, where Dagster runs are typically managed in ephemeral, single-purpose computational resources (e.g. processes, Kubernetes Jobs) responsible for task scheduling. Those in turn can decide on their own task scheduling policy. In the same Dagster deployment, one run that doesn’t do much computation could execute in-process on a lightweight node, whereas another run could delegate task execution to heavier clusters. Many users have found that with that run-level parallelism is sufficient, and without that flexibility they would have had to manage more burdensome centralized infrastructure.
...
- **Longitudinal Views**: Users can track and view the ––e.g. number of rows, time it took to generate it––of a particular asset over time.
