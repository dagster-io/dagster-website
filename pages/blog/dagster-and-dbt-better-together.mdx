...
For users who already speak SQL, dbt’s tooling is unparalleled. Adopting Dagster doesn’t mean  replacing dbt. Dagster enters the picture when you need to orchestrate dbt alongside other tools, from Spark jobs to Jupyter notebooks and more.
...
The following example synchronously runs a dbt project via RPC and logs the results of each model of the dbt project that was executed.
...
# Logs the execution result of each dbt model.
@solid
def log_models(context, poll_result: DbtRpcPollResult):
    model_results = poll_result.results
    for model_result in model_results:
        context.log.info(
            "dbt Model {alias} completed in {execution_time:.3f}s with status: {status}".format(
                alias=model_result.node["alias"],
...
def dbt_rpc_pipeline():
    log_models(dbt_rpc_run_and_wait())
...
```bash
> dbt Model sort_by_calories completed in 0.124s with status: CREATE VIEW
> dbt Model least_caloric completed in 0.148s with status: CREATE VIEW
```
...
Using `context.log` is useful for debugging, but doesn’t provide persistent records of dbt run results. Luckily, in `dagster-dbt`, the CLI solids also emit assets via `AssetMaterialization`s that persist across multiple executions of the Dagster pipeline.
