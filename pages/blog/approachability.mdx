...
Many of the changes are aimed at improving Dagster’s *progressive disclosure of complexity.*  Dagster includes a large array of concepts that help model and organize the complexity of the data development and deployment lifecycle.  Concepts that feel vital when you're building a complex data application can feel heavyweight when you're just trying to get a basic pipeline up and running.  You want software that grows with you.  Progressive disclosure of complexity is about making it simple to do simple things and having advanced concepts should show up only when you need them.
