...
We adopted Dagster for our core address data processing flows because it gives
us a great developer experience while providing the backwards compatibility we
needed with our legacy Airflow systems.
...
We needed a solution that would allow for incremental adoption on top of our
...
Dagster pipelines them into Airflow DAGs that can be deployed on our existing
scheduler instances.

This is especially critical for us since our Dagster pipelines need to
interoperate with legacy Airflow tasks and DAGs. (If we starting from scratch,
we would just run the Dagster scheduler directly.)
...
Most importantly, as we're developing our solids, we can work with them in Dagit
...
production infrastructure. As a consequence, tests are much less costly, because
they can be run on dev-appropriate infrastructure.
