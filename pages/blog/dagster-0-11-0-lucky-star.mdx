...
Many data pipelines include steps that have to be repeated once for each element of some input. Very often, you don't know in advance how many inputs youre going to have to process. And once you're done, you'll often want to collect the outputs of each process for downstream processing as an aggregate.
...
From the standpoint of the orchestration engine, we think of this as the simplest kind of _dynamic orchestration_ -- where the work to be orchestrated is determined not at pipeline definition time but at runtime, dependent on data that's observed as part of pipeline execution.
...
Since the `map` and `collect` operations are part of the pipeline definition, we can visualize them in Dagit. Both the mapped dynamic output and the collected input have special sigils and the mapped solid is embellished in the pipeline view to illustrate that it will be executed dynamically. 
...
Keep in mind that Dagster is not a specialized distributed computation framework like [Dask](https://dask.org/) or [Spark](https://spark.apache.org/). If you need to process very large datasets efficiently in parallel, we still recommend that you use Dagster to orchestrate solids that perform compute in a specialized framework. But for many coarse-grained cases, these abstractions make it easier to express logical parallelism and write pipelines that get work done.
...
You can drill down into an individual partition to see the history of all runs executed for that partition:
...
We've already told Dagster about the data dependencies between the inputs and outputs of our solids -- that's how we constructed the pipeline. When we attach asset keys to the outputs of our solids, we give Dagster the information it needs to infer the lineage graph that connects the associated assets.
...
<p align="center">
  <img
    src="/posts/dagster-0-11-0-lucky-star/docs_search.png"
